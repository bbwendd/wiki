...
正式交付後，必須依 `wiki/workflows/交付封存流程.md` 保存交付物、來源、ABCDE 結論與後續待辦。
...
複雜任務應優先套用 `schema/templates/subagent_task.md` 進行任務指派。

## Connector 規則

需要外部資料或工具時，先查 `schema/MCP_CONNECTOR_MAP.md`。所有 connector 只能用於讀取、整理、草稿或準備操作；寄信、傳訊息、改行程、刪檔、分享權限或對外承諾前，仍必須再次取得使用者明確確認。
